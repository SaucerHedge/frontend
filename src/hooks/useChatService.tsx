...
import { useAppKitContext } from "@/contexts/AppKitContext";
...
const initialChatMessages: ChatMessageData[] = [
  {
    type: "bot",
    message:
      "Welcome to SaucerHedge! I'm your intelligent DeFi advisor powered by Hedera's AI Agent Kit. I can help you discover yield opportunities, analyze risks, execute strategies, and manage your portfolio.\n\n🤖 **Agent Status**: Connecting to advanced DeFi capabilities...\n\nHow can I assist you today?",
    timestamp: "Just now",
    suggestions: [
...
export const useChatService = () => {
  ...
  const convertAgentMessage = useCallback(
    (agentMsg: AgentMessage): ChatMessageData => {
      return {
        type:
          agentMsg.type === "agent"
            ? "bot"
  ...
  useEffect(() => {
    const statusMessage = agent.status.error
      ...
      ? "✅ **Agent Ready**: Connected to SaucerHedge with full DeFi capabilities"
      : "🔄 **Agent Status**: Connecting to advanced DeFi capabilities...";

    const updatedInitialMessage: ChatMessageData = {
      ...initialChatMessages[0],
      message: `Welcome to SaucerHedge! I'm your intelligent DeFi advisor powered by Hedera's AI Agent Kit. I can help you discover yield opportunities, analyze risks, execute strategies, and manage your portfolio.\n\n${statusMessage}\n\nHow can I assist you today?`,
    };
    ...
    if (agent.isReady) {
      ...
      setTimeout(() => {
        const fallbackResponse: ChatMessageData = {
          type: "bot",
          message: `I'm currently connecting to the SaucerHedge agent to process your request: "${message}"\n\n🔄 **Status**: ${
            agent.status.error || "Establishing connection..."
