...
export const useAgent = () => {
  ...
  const connect = useCallback(async () => {
    try {
      ...
      if (!isHealthy) {
        throw new Error("SaucerHedge server is not ready");
      }
      ...
      addSystemMessage(
        "Connected to SaucerHedge Agent. Ready to assist with your DeFi operations!"
      );
    } catch (error) {
      const errorMessage =
  ...
  const disconnect = useCallback(() => {
    ...
    addSystemMessage("Disconnected from SaucerHedge Agent");
  }, [addSystemMessage]);
